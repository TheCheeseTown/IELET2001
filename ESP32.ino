...
const char *SUBSCRIBE_DEVICE_LABEL = ""; //Navnet på den kanalen ubidots publiserer slider-verdiene til

const int PUBLISH_FREQUENCY = 5000; // Hvor ofte skal det publiseres. 

unsigned long timer;
uint8_t led = 25; //LED på pinne 25
int moisturePin = 32; //Fuktighetssensor på pinne 32
...
{ 
  ...
  {
    double pubtemp = ListToAverage(tmpList, 2); // regner ut gjsn av temp målinger
    double pubMoisture = ListToAverage(moistureList,2); // regner ut gjsn av soilmoisture målinger
    double pubMoistureProsent = map(pubMoisture,0,4095,0,100); // mapper gjsn av soilmoisture til prosent
    float AIRHUMIDITY = bme.readHumidity(); // leser av luftfuktighet
    ...
    ubidots.add("tmp", pubtemp); //Emne den sender gitt verdi til
    ubidots.add("moisture",pubMoistureProsent);
    ubidots.add("airmoisture",AIRHUMIDITY);
    ubidots.publish();//Publiserer med MAC som "device label", dette gjør at hver enhet er unik. 
    delay(500);
  }
